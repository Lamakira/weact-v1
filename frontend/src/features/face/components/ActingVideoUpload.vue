...
interface Props {
  videoInfo: ActingVideoInfo | null
  isUploading?: boolean
  isDeleting?: boolean
  error?: string | null
...
const displayThumbnail = computed(() => {
  ...
  return props.videoInfo?.acting_video_thumbnail_url ?? null
})

const displayVideoUrl = computed(() => {
  return props.videoInfo?.acting_video_url ?? null
})
...
watch(
  () => props.videoInfo?.acting_video_url,
  () => {
    if (previewUrl.value) {
...
<template>
  <div class="acting-video-upload" data-testid="acting-video-upload">
    <!-- Error message -->
    <div
      v-if="error"
    ...
    <div class="flex flex-col items-center gap-4">
      ...
      <div class="flex gap-3">
        ...
        <button
          ...
          aria-label="Supprimer la vidéo d'acting"
          @click="handleDelete"
    ...
    <ConfirmModal
      ...
      message="Êtes-vous sûr de vouloir supprimer cette vidéo d'acting ? Cette action est irréversible."
