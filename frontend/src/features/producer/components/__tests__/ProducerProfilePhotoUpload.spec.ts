...
import ProducerProfilePhotoUpload from '../ProducerProfilePhotoUpload.vue'
import type { Producer } from '../../types'

describe('ProducerProfilePhotoUpload', () => {
  ...
  it('renders placeholder when no profile photo', () => {
    const wrapper = mount(ProducerProfilePhotoUpload, {
      props: {
        profile: null,
      },
    })
  ...
  it('renders profile photo when available', () => {
    ...
    expect(img.attributes('src')).toBe(mockProducer.profile_photo_url)
  })
  ...
  it('shows delete button only when photo exists', () => {
    const wrapperWithPhoto = mount(ProducerProfilePhotoUpload, {
      props: {
        profile: mockProducer,
      },
    })

    const wrapperWithoutPhoto = mount(ProducerProfilePhotoUpload, {
      props: {
        profile: null,
      },
    })
  ...
  it('clears preview and shows original photo when error occurs after file selection', async () => {
    ...
    expect(wrapper.find('[data-testid="profile-photo"]').attributes('src')).toBe(
      mockProducer.profile_photo_url,
    )
    ...
    expect(photo.attributes('src')).toBe(mockProducer.profile_photo_url)
  })
  ...
  describe('Producer type variations', () => {
    it('handles particulier producer type', () => {
      ...
      const wrapper = mount(ProducerProfilePhotoUpload, {
        props: {
          profile: particulierProducer,
        },
      })

      expect(wrapper.find('[data-testid="profile-photo"]').exists()).toBe(true)
      expect(wrapper.find('[data-testid="profile-photo"]').attributes('src')).toBe(
        particulierProducer.profile_photo_url,
      )
    })
  })
})
