...
/**
 * Composable for fetching and managing Producer dashboard statistics
 */
export function useProducerDashboardStats() {
  const stats = ref<ProducerDashboardStats | null>(null)
  ...
  // Computed: total missions (excluding drafts)
  const totalMissions = computed(() => {
    if (!stats.value) return 0
    return stats.value.published + stats.value.closed + stats.value.completed
  })
  ...
  async function fetchStats(): Promise<void> {
    ...
    try {
      const response = await dashboardApi.getProducerStats()
      ...
      if (isAxiosError(err)) {
        if (err.response?.status === 401) {
          ...
          error.value = 'Accès réservé aux Producteurs'
