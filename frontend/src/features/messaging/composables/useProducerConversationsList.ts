...
/**
 * Composable for managing Producer conversations list state
 * Handles loading paginated conversations with latest message previews
 */
export function useProducerConversationsList() {
  ...
  /**
   * Load conversations from API (Producer endpoint)
  ...
  async function loadConversations(page = 1): Promise<boolean> {
    ...
    try {
      const response = await messagingApi.getProducerConversations(page)
  ...
  async function loadMoreConversations(): Promise<boolean> {
    ...
    try {
      const response = await messagingApi.getProducerConversations(nextPage)
