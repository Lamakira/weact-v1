...
/**
 * Composable for sending messages in a conversation (Producer)
 * Handles API call, loading state, and validation errors
 */
export function useSendProducerMessage() {
  const isSending = ref(false)
  const error = ref<string | null>(null)
  const validationErrors = ref<string[]>([])

  /**
   * Send a message in a conversation (Producer endpoint)
  ...
  async function sendMessage(conversationId: string, content: string): Promise<Message | null> {
    ...
    try {
      const response = await messagingApi.sendProducerMessage(conversationId, {
