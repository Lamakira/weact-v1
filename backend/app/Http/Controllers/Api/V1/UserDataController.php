...
class UserDataController extends Controller
{
    ...
    public function export(Request $request): JsonResponse
    {
        ...
        if ($user->userable_type === Face::class && $user->userable) {
            ...
            // Experiences
            $data['experiences'] = $face->experiences()->get()->map(fn ($exp) => [
                'titre' => $exp->titre,
            ...
            // Candidatures
            $data['candidatures'] = $face->candidatures()
        ...
        if ($user->userable_type === Producer::class && $user->userable) {
            ...
            // Missions
            $data['missions'] = $producer->missions()->get()->map(fn ($m) => [
                'titre' => $m->titre,
                'status' => $m->status,
        ...
        $data['bookings'] = $bookingsRelation->get()->map(fn ($b) => [
            'status' => $b->status,
            'amount' => $b->amount,
        ...
        // Ratings given
        $data['ratings_given'] = $user->ratingsGiven()->get()->map(fn ($r) => [
            'score' => $r->score,
            'comment' => $r->comment,
