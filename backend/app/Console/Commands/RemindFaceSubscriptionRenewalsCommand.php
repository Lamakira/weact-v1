...
class RemindFaceSubscriptionRenewalsCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        foreach ($stale30 as $subscription) {
            try {
                $changed = DB::transaction(function () use ($subscription, $window30Start, $window30End): bool {
                ...
                if ($changed) {
                    $this->info("30-day reminder sent for face subscription #{$subscription->id}");
                    $sent30++;
                }
            } catch (\Throwable $e) {
        ...
        foreach ($stale7 as $subscription) {
            try {
                $changed = DB::transaction(function () use ($subscription, $window7Start, $window7End): bool {
                    /** @var FaceSubscription $locked */
                    $locked = FaceSubscription::lockForUpdate()->findOrFail($subscription->id);
                ...
                if ($changed) {
                    $this->info("7-day reminder sent for face subscription #{$subscription->id}");
                    $sent7++;
                }
            } catch (\Throwable $e) {
    ...
    private function sendReminder(FaceSubscription $subscription, int $daysRemaining): void
    {
        ...
        Notification::create([
            ...
            'data' => [
                'message' => "Rappel : votre abonnement Premium annuel expire dans {$daysRemaining} jours (le {$expiresLabel}). Renouvelez pour garder vos photos 3-4 et votre vidéo de jeu publiques.",
                'face_subscription_id' => $subscription->id,
